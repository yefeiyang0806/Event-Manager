...
<html lang="en">
  <head>
    ...
    <script>
  		$(function() {
  	    	$( "#datepicker" ).datepicker({ minDate: "-99Y", maxDate: "0", dateFormat: "yy-mm-dd", changeMonth: true, changeYear: true});
  	  	});
  	</script>
	...
	<br /><br /><br /><br />
	  <div class="row">
	    <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
		  <form role="form" method=POST action="/project/user/create">
			<h2>Please Register</h2>
			<c:if test="${ not empty error_msg }">
			  <br />
			  <div class="panel panel-danger">
                <div class="panel-heading">
                  <h3 class="panel-title">Creating Failed</h3>
                </div>
...
<!-- 			  <h3>Account Info</h3><br/> -->
			...
			<div class="form-group">
			  <c:if test="${not empty user.username}">
				<input type="text" name="username" class="form-control input-lg" value="${ user.username }" placeholder="Username"/>
			  </c:if>		
			...
			<div class="row">
			  <div class="col-xs-12 col-sm-6 col-md-6">
			    ...
			    <c:if test="${empty user.password}">
			      <input type="password" name="password" class="form-control input-lg" placeholder="Password Here">
			    </c:if>
			    <c:if test="${not empty user.password}">
			      <input type="password" name="password" class="form-control input-lg" placeholder="Password Here" value="${ user.password }">
			    </c:if>
			    </div>
			  </div>
			  <div class="col-xs-12 col-sm-6 col-md-6">
			    <div class="form-group">
			    <c:if test="${empty password2}">
			      <input type="password" name="password2" class="form-control input-lg" placeholder="Confirm Password">
			    </c:if>
			    <c:if test="${not empty password2}">
			      <input type="password" name="password2" class="form-control input-lg" placeholder="Confirm Password" value="${ password2 }">
			    </c:if>
			    </div>
			  </div>
			</div>
...
<!-- 			  <h3>Personal Details</h3><br/> -->
			...
			<div class="row">
				<div class="col-xs-12 col-md-6" style="margin-left:25%;">
				  <input type="submit" id="register" class="btn btn-primary btn-block btn-lg" value="Register">
				</div>
			</div>
		  </form>
		</div>
